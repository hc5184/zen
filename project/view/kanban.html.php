...
<div id='kanban'>
  ...
  <table class='boards-layout table active-disabled table-bordered' id='kanbanWrapper'>
    ...
    <tbody>
      <?php $rowIndex = 0; ?>
      <?php foreach($stories as $storyKey => $story):?>
        <?php if(count(get_object_vars($story)) == 0 or $story->status == 'closed') continue;?>

        <?php if(!(empty($story->id) and empty($story->tasks['wait']) and empty($story->tasks['doing']) and empty($story->tasks['done']))):?>
            ...
            <?php if($hasStory):?>
              <td class='col-story'>
                ...
                <?php else:?>
                  <div class='board board-story stage-<?php echo $story->stage?><?php if($story->assignedTo == $this->app->user->account) echo ' inverse';?>' data-id='<?php echo $story->id?>'>
                    <div class='board-title'>
                      <?php echo html::a('', 'STORY' . $story->title , '', 'class="kanbanFrame" title="' . $story->title . '"');?>
                  </div>
                <?php endif;?>
              </td>
            <?php endif;?>
                ...
                <?php if(!empty($story->tasks[$col])):?>
                  ...
                  <div class='board board-task board-task-<?php echo $col ?><?php if($task->assignedTo == $this->app->user->account) echo ' inverse';?>' data-id='<?php echo $task->id?>' id='task-<?php echo $task->id?>'>
                    <div class='board-title'>
                      ...
                      echo html::a($this->createLink('task', 'view', "taskID=$task->id", '', true), '['.$lang->task->typeList[$task->type].'] '.$task->name, '', 'class="kanbanFrame" title="' . $task->name . '"');
                      ?>
                      <div class='board-actions'>
                        <button type='button' class='btn btn-mini btn-link btn-info-toggle'><i class='icon-angle-down'></i></button>
                        <div class='dropdown'>
        ...
        <?php if(!empty($story->bugs) and (!empty($story->bugs['wait']) or !empty($story->bugs['done']))):?>
          <tr data-id='<?php echo $rowIndex++?>'>
            <?php if($hasStory):?>
              <td class='col-story'>
                <?php if(!empty($story->id)):?>
                ...
                <?php else:;?>
                  <div class='board board-story stage-<?php echo $story->stage?><?php if($story->assignedTo == $this->app->user->account) echo ' inverse';?>' data-id='<?php echo $story->id?>'>
                    <div class='board-title'>
                      <?php echo html::a('', 'BUG', '', 'class="kanbanFrame" title="BUG"');?>
                    </div>
                  </div>
                <?php endif;?>
              </td>
